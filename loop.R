...
View(files)
birds_2008 <- stack(dat)
total <- calc(birds_2008, sum)


cumul_ap <- c(cropped_raster, dat[[3]], along = "z")
View(cumul_ap)
cumul_sum <- st_apply(cumul_ap, c(1,2), sum)

#####grid and plot, for cumulative sum
brange_small <- st_transform(brange_small, crs = st_crs(cumul_sum))
ext <- st_bbox(brange_small)
c <- st_crop(cumul_sum, ext)
x=c
x <- st_as_stars(c)
x2 = log(x+1)
x3 <- x2
